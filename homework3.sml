...
fun applyEye (VRat (i,1)) = VMat (identityMat i)
 ...
 *                  expr                             [expr_list_EXPR]
 * 
 *   expr_rows ::= expr_row T_SEMICOLON expr_rows    [expr_rows_MULTIPLE]
 *                 expr_row                          [expr_rows_SINGLE]
 *
 *   expr_row ::= expr expr_row                      [expr_row_MULTIPLE]
 *                expr                               [expr_row_SINGLE]
 *                  
...
and parse_expr_rows ts = 
    (case parse_expr_rows_MULTIPLE ts
        of NONE => 
        (case parse_expr_rows_SINGLE ts
            of NONE => NONE
            | s => s)
        | s => s)

and parse_expr_rows_MULTIPLE ts = 
    (case parse_expr_row ts
        ...
        (case expect_SEMICOLON ts
            ...
            (case parse_expr_rows ts
                of NONE => NONE
                | SOME (rows, ts) => SOME (row::rows, ts))))

and parse_expr_rows_SINGLE ts =
    (case parse_expr_row ts
        of NONE => NONE
        | SOME (row, ts) => SOME ([row], ts))

and parse_expr_row ts = 
    (case parse_expr_row_MULTIPLE ts
        of NONE =>
        (case parse_expr_row_SINGLE ts
            of NONE => NONE
            | s => s)
        | s => s)

and parse_expr_row_MULTIPLE ts =
    (case parse_expr ts
        of NONE => NONE
        | SOME (e, ts) =>
        (case parse_expr_row ts
            of NONE => NONE
            | SOME (row, ts) => SOME (e::row, ts)))

and parse_expr_row_SINGLE ts =
    (case parse_expr ts
        of NONE => NONE
        | SOME (e, ts) => SOME ([e], ts))
